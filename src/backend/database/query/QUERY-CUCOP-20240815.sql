...
CREATE TABLE `quotation_products` (
  ...
  `subtotal` FLOAT GENERATED ALWAYS AS (quantity * price * (1-discount)) STORED,
  `IVA` FLOAT NOT NULL,
  `amountIVA` FLOAT GENERATED ALWAYS AS (subtotal * IVA) STORED,
  `ISR` FLOAT NOT NULL,
  `amountISR` FLOAT GENERATED ALWAYS AS (subtotal * ISR) STORED,
  `totalPrice` FLOAT GENERATED ALWAYS AS (subtotal + amountIVA + amountISR) STORED,
  `active` tinyint(1) NOT NULL DEFAULT 1,
  `createdAt` timestamp NULL DEFAULT CURRENT_TIMESTAMP,
  `updatedAt` timestamp NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
...
INSERT INTO SUPPLIERS (name, description, tin, phone, address, email, active)
 VALUES
